...
with st.sidebar:  
      ...
      valor = st.number_input("Valor", min_value=50000, max_value=250000, value=60000)
      ...
      if st.button("Processar"):
         ...
         for i in range(int(prazo)):
            ...
            if i % 12 == 0 and i > 0:               
               ...
               y=valor_alu+((valor_alu*avaria)*10)
               alugar.append(y)
               continue
            alugar.append(x)
         ...
         for i in range(int(prazo)): 
            x=float(price_par[1])   
            if i == 0:
               x=x+premissa["1º Licenciamento"]
            if i % 12 == 0 and i > 0: 
               x=x+premissa["Licenciamento"] 
               valor = valor + (valor*premissa["Reajuste"])
               cont = 1
            if cont >=1 :
               if cont <=4 :
                  if price_par[0]=="PickUp":
                     x=x+((valor*premissa["IPVA-Strada"])/4)                     
                     x=x+((valor*premissa["Seguro"])/10)
                  if price_par[0]=="Popular":
                     x=x+((valor*premissa["IPVA"])/4)
                     x=x+((valor*premissa["Seguro"])/10)
               if cont>4<=10:     
                  x=x+((valor*premissa["Seguro"])/10) 
               cont = cont + 1
